...
$u_search	= 'rss.' . $phpEx . ( empty($rss_mode) ? '' : '?mode='.$rss_mode ) . ( empty($forum_id) ? '' : '?f='.$forum_id ) . ( empty($topic_id) ? '' : '&amp;t='.$topic_id );
...
if($result = $db->sql_query($sql))
{
	...
	$result = $db->sql_query_limit($sql, $total_count, $start);
}
...
while($row = $db->sql_fetchrow($result))
{
	$forum_data = get_forum_data($row['forum_id']);
	...
	if($auth->acl_get('f_read', $forum_data['forum_id']))
	{
		...
		$template->assign_block_vars('items', array(
			'TITLE'			=> $row[$row_title],
			'LINK'			=> $item_link,
			'DESCRIPTION'	=> ( $row_text != '' ) ? fix_content($row[$row_text], $row[$row_uid], $row[$row_bit], false) : '',
			'STATISTICS'	=> $stats,
			'PUBDATE'		=> date3339($row[$row_date], $config['board_dst']),
			'CATEGORY'		=> "$board_url/viewforum.$phpEx?f={$forum_data['forum_id']}",
			'CATEGORY_NAME'	=> $forum_data['forum_name'],
...
$template->assign_vars(array(
	...
	'STAT_TIMEZONE'			=> ($user->data['user_dst'] || ($user->data['user_id'] == ANONYMOUS && $config['board_dst'])) ? sprintf($user->lang['ALL_TIMES'], $user->lang['tz'][$tz], $user->lang['tz']['dst']) : sprintf($user->lang['ALL_TIMES'], $user->lang['tz'][$tz], ''),
	...
	'PAGINATION'			=> generate_pagination($u_search, $total_count, $per_page, $start),
...
function fix_content($content, $uid, $bitfield, $html = false)
{
	...
	if ( $show_text && !empty($content) )
	{
		...
		if ( $text_limit != 0 && strlen($content) > $text_limit)
		{
			$content = substr($content, 0, $text_limit) . ' [....]';
		}
	}
