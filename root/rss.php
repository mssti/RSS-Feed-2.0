...
* @version: $Id: rss.php, v 1.0.5 2009/01/03 03:01:09 leviatan21 Exp $
...
switch ($rss_mode)
{
	case 'forums':
		...
		$sql = 'SELECT f.forum_id, f.forum_password, f.forum_topics, f.forum_posts, f.forum_name, f.forum_last_poster_id, f.forum_desc, f.forum_desc_bitfield, f.forum_desc_uid, f.forum_last_post_time 
				FROM ' . FORUMS_TABLE . ' f 
...
while ($row = $db->sql_fetchrow($result))
{
	...
	if ( ( $auth->acl_get('f_read', $row['forum_id']) ) && !$row['forum_password'] )
	{
		switch ($rss_mode)
		{
			case 'forums':
				...
				$item_link	= append_sid("$board_url/viewforum.$phpEx", "f={$row['forum_id']}", true, false);
			...
			case 'topics':
				...
				$item_link	= append_sid("$board_url/viewtopic.$phpEx", "f={$row['forum_id']}&amp;t={$row['topic_id']}&amp;view=unread#unread", true, false);
				$user_link	= '<a href="' . append_sid("$board_url/memberlist.$phpEx", "mode=viewprofile&amp;u=" . $row['topic_poster'], true, false) . '">' . $row['topic_first_poster_name'] . '</a>';
			...
			case 'posts':
				...
				$item_link	= append_sid("$board_url/viewtopic.$phpEx", "p={$row['post_id']}#p{$row['post_id']}", true, false);
				$user_link	= '<a href="' . append_sid("$board_url/memberlist.$phpEx", "mode=viewprofile&amp;u=" . $row['poster_id'], true, false) . '">' . $row['username'] . '</a>';
			...
			default:
				...
				$item_link	= append_sid("$board_url/viewtopic.$phpEx", "p={$row['post_id']}#p{$row['post_id']}", true, false);
				$user_link	= '<a href="' . append_sid("$board_url/memberlist.$phpEx", "mode=viewprofile&amp;u=" . $row['user_id'], true, false) . '">' . $row['username'] . '</a>';
				$stats		= $user->lang['POSTED'] . ' ' . $user->lang['POST_BY_AUTHOR'] . ' ' . $user_link . ' &bull; ' . $user->lang['POSTED_ON_DATE'] . ' ' . $user->format_date($row['topic_time']). ' &bull; ' . $user->lang['REPLIES'] . ' ' . $row['topic_replies'] . ' &bull; ' . $user->lang['VIEWS'] . ' ' . $row['topic_views'];
				break;
		}
		...
		$template->assign_block_vars('items', array(
			...
			'PUBDATE'		=> date2822($row[$row_date]),
			'CATEGORY'		=> "$board_url/viewforum.$phpEx?f={$row['forum_id']}",
			'CATEGORY_NAME'	=> utf8_htmlspecialchars($row['forum_name']),
			'AUTHOR'		=> ( (isset($row['user_email'])) ? $row['user_email'] : '' ) . ' (' .( (isset($row_username)) ? $row[$row_username] : $row[$row_creator] ) . ')',
			'GUID'			=> $item_link,
		));
	}
}
...
function generate_content( $content, $uid, $bitfield )
{
	...
	/* Commented out so do not loose the code.
		// Resize images ?
...
function date2822( $forced = false, $timestamp = 0, $timezone = 0 )
{
	...
	$timezone  = ( (int) $timezone == 0 ) ? 'GTM' : $timezone;
