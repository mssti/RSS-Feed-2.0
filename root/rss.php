...
//			$topic_sql = ($rss_t_id == 0) ? ' AND p.post_id = t.topic_last_post_id ' : " AND p.topic_id = t.topic_id AND t.topic_id = $rss_t_id";
			...
			$sql_array['WHERE'] = 't.topic_moved_id = 0 AND p.post_approved = 1 ';
			$sql_array['WHERE'].= ' AND ( f.forum_id = p.forum_id ';
...
**/
function EXrss_append_sid($url, $params)
{
...
function rss_append_sid($url, $params)
{
	...
	**/
	...
	$url = str_replace( generate_board_url() . '/', $phpbb_root_path, $url);
...
/*
		...
		$content = enable_html($content, $uid);
		// END Enable HTML
*/
		// Fix smilies
